...
module.exports = {
    ...
    execute(message, args){
        ...
        if(randomNum == 1){
        message.channel.send(`IT'S PEGGING TIME, @${blamizar#5208}`, l1);
        message.delete({ timeout: 1000 });

    } else if(randomNum == 2){
        message.channel.send(`THAT ASS IS MINE, @${blamizar#5208}`, l2);
        message.delete({ timeout: 1000 });
    } else if(randomNum == 3){
            message.channel.send(`GET OVER HERE @${blamizar#5208}, IT'S PEGGIN' TIME`, l3);
            message.delete({ timeout: 1000 });
    } else if(randomNum == 4){
        message.channel.send(`TIME FOR A PEGGING SESH, @${blamizar#5208}!`, l4);
        message.delete({ timeout: 1000 });
}
}
};